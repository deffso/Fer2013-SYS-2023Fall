...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, values, keys, query, mask):
        values = values.reshape(
            values.shape[0], -1, self.num_heads, self.head_dim
        )
        keys = keys.reshape(keys.shape[0], -1, self.num_heads, self.head_dim)
        ...
        out = torch.einsum("nhql,nlhd->nqhd", [attention, values])#.reshape(query.shape[0], -1, self.num_heads * self.head_dim)

        out = out.view(query.shape[0], -1, self.head_dim)

        out = self.fc_out(out)
        return out

# 定义带有多头注意力模块的残差块
class ResidualBlockWithMultiHeadAttention(nn.Module):
    def __init__(self, embed_size, num_heads):
        super(ResidualBlockWithMultiHeadAttention, self).__init__()
        ...
        self.global_avg_pooling = nn.AdaptiveAvgPool1d(1)  # 添加全局平均池化层
        self.fc_out = nn.Linear(embed_size, 7)
    def forward(self, x, mask=None):
        ...
        x = self.norm2(x)
        x = self.global_avg_pooling(x.permute(0, 2, 1)).squeeze(2)  # 全局平均池化
        x = self.fc_out(x)  # 注意这里的修改
        return x
...
# 在ResNet-18中间的某个位置插入多头注意力模块
# 假设在第三个残差块后插入多头注意力
resnet18.layer3[0] = ResidualBlockWithMultiHeadAttention(256, num_heads=8)

# 修改全连接层以输出所需的类别数num_classes = 7
...
month = now.month
day = now.day
log_file_path = 'training_logs_MutiHead'+str(month)+'月'+str(day)+'日'+str(hour)
